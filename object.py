...
class Geometry(object):
    ...
    def rotate(self,attitude: Attitude, step_width):
        if step_width == 90:
            # 原始的 8 个点（立方体的对角顶点）
            corners = np.array([
                [0, 0, 0],
...
class Item(object):
    ...
    def rotate(self, attitude: Attitude, step_width):
        ...
        self.curr_geometry.rotate(attitude, step_width)
        self.attitude = attitude

    # 包括旋转和平移的变换
    def transform(self, transform: Transform, step_width):
        self.rotate(transform.attitude, step_width)
        ...
        if step_width == 90:
            ...
            for roll in range(0, 360, step_width):
                for pitch in range(0, 360, step_width):
                    ...
                    self.curr_geometry = Geometry(self.init_geometry.cube)
                    self.curr_geometry.rotate(curr_attitude, step_width)
                    # 计算当前姿态对应的稳定性
